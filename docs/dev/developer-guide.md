...
All non-flag arguments are extracted and then used to lookup via [plugin.PluginManager.FindPlugin()]() in the plugin directory (and the execution `$PATH` if configured) calculated in the _Bootstrap_ phase.
