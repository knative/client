...
func (test *e2eTest) revisionListWithService(t *testing.T, serviceNames ...string) {
	for _, svcName := range serviceNames {
		confGenStr := test.findConfigurationGeneration(t, svcName)
		...
		for _, line := range outputLines[1 : len(outputLines)-1] {
			// Skip checking of a revision name line-by-line because currently we don't have a good way to capture revision names
			// randomly generated by server side
			// TODO(toVersus): check revision name once `kn service update --revision-name revName` is merged
			assert.Check(t, util.ContainsAll(line, svcName, confGenStr))
			confGenStr = strconv.Itoa(confGen - 1)
		}
	}
}
...
func (test *e2eTest) findConfigurationGeneration(t *testing.T, serviceName string) string {
	revGen, err := test.kn.RunWithOpts([]string{"revision", "list", "-s", serviceName, "-o=jsonpath={.items[0].metadata.labels.serving\\.knative\\.dev/configurationGeneration}"}, runOpts{})
	assert.NilError(t, err)
	if revGen == "" {
