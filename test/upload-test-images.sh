...
function upload_test_images() {
  ...
  local serving_image_dir="knative.dev/serving/test/test_images"
  local docker_tag=$1
  local tag_option=""
  ...
  #build and publish images from serving
  ko resolve --jobs=4 ${platform} ${tag_option} -RBf "${serving_image_dir}" > /dev/null
}
