...
function install_istio() {
  ...
  (
    cd $NET_ISTIO_DIR \
      && git init \
      && git remote add origin https://github.com/knative-extension/net-istio.git \
...
function knative_setup() {
  ...
  if [ "${serving_version}" = "latest" ]; then
    ...
    start_knative_serving "https://storage.googleapis.com/knative-releases/serving/previous/v${serving_version}/serving-crds.yaml" \
      ...
      "https://storage.googleapis.com/knative-releases/net-istio/previous/v$(get_latest_release_version "knative-extension" "net-istio")/net-istio.yaml"
  fi
