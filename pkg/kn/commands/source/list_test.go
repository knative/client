...
func TestSourceListTypes(t *testing.T) {
	output, err := sourceFakeCmd([]string{"source", "list-types"},
		...
		newSourceCRDObjWithSpec("apiserversources", "sources.eventing.knative.dev", "v1alpha1", "ApiServerSource"),
	)
...
func TestSourceList(t *testing.T) {
	output, err := sourceFakeCmd([]string{"source", "list"},
		...
		newSourceUnstructuredObj("a1", "sources.eventing.knative.dev/v1alpha1", "ApiServerSource"),
	...
	assert.Check(t, util.ContainsAll(output[1], "a1", "ApiServerSource", "apiserversources.sources.eventing.knative.dev", "svc:foo", "<unknown>"))
