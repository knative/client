...
func TestServiceExportwithMultipleRevisions(t *testing.T) {
	for _, tc := range []testCase{{
		...
		latestSvc: getServiceWithOptions(
			...
			withConfigurationAnnotations(map[string]string{"client.knative.dev/user-image": "murugappans/goswaggertest:v2"}),
		...
		expectedSvcList: getServiceListWithOptions(
			withServices(
				...
				withConfigurationAnnotations(map[string]string{"client.knative.dev/user-image": "murugappans/goswaggertest:v1"}),
				withServicePodSpecOption(withContainer()),
		...
		revisionList: getRevisionListWithOptions(
			withRevisions(
				...
				withRevisionAnnotations(map[string]string{"client.knative.dev/user-image": "murugappans/goswaggertest:v1"}),
			...
			withRevisions(
				...
				withRevisionAnnotations(map[string]string{"client.knative.dev/user-image": "murugappans/goswaggertest:v2"}),
				withRevisionPodSpecOption(withContainer()),
			),
		),
